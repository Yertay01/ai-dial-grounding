...
# Pydantic models for search parameters
...
class SearchRequests(BaseModel):
    ...
    search_request_parameters: list[SearchRequest] = Field(default=[], description="List of search parameters")
...
def augment_prompt(user_question: str, context: list[dict[str, Any]]) -> str:
    ...
    context_str = []
    for user in context:
        context_str.append("User:")
        for key, value in user.items():
            context_str.append(f"  {key}: {value}")
        context_str.append("")  # Empty line between users
    formatted_context = "\n".join(context_str)
    
    # 2. Make augmentation for USER_PROMPT
    augmented = USER_PROMPT.format(context=formatted_context, query=user_question)
