...
def join_context(context: list[dict[str, Any]]) -> str:
    """Convert user data from list of dicts to formatted string."""
    result = []
    for user in context:
        result.append("User:")
...
async def main():
    ...
    if user_question:
        ...
        # 1. Get all users using UserClient
        user_client = UserClient()
        all_users = user_client.get_all_users()
        
        # 2. Split all users into batches (100 users per batch)
        batch_size = 100
        user_batches = [all_users[i:i + batch_size] for i in range(0, len(all_users), batch_size)]
        print(f"Processing {len(all_users)} users in {len(user_batches)} batches\n")
        
        # 3. Prepare tasks for async run of response generation
        tasks = []
        ...
        # 6. Check if we have any results
        if filtered_results:
            ...
            print(f"\nNo users found matching your query: '{user_question}'")
